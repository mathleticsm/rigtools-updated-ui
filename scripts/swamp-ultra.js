if (
    (alert(
      "This was patched!!!! If you are seeing this, hard-disable NOW and powerwash to undo!",
    ),
    "haldlgldplgnggkjaafhelgiaglafanh" !== document.domain)
  )
    throw "[swamp] can only be used in the GoGuardian runtime.";
    ...
    swamp = {
      ...
      elements: {
        create(e, t) {
          ...
          for (var a in e)
            o[a] = a.startsWith("on") ? swamp.functions[e[a]] : e[a];
          !e.kids && i[o.id] && (o.innerHTML = i[o.id]),
      ...
      functions: {
        ...
        log_replace(e) {
          (swamp.elements.output.textContent += "\n\n" + e),
            (swamp.elements.output.scrollTop =
              swamp.elements.output.scrollHeight);
        },
        run_code() {
        ...
        hard_disable() {
          for (var e = 0; e < localStorage.length; e++)
            localStorage.key(e).startsWith("swamp") ||
              (localStorage[localStorage.key(e)] = this.undo ? "" : "-");
          swamp.background.location.reload(),
            this.undo && swamp.functions.reload();
        },
        soft_disable() {
          ...
          chrome.management.getAll(function (e) {
            e.forEach(function (e) {
              swamp.elements.create(
                ...
                swamp.functions.strikethrough(swamp.elements[e.id], e.enabled),
                e.id === chrome.runtime.id &&
                  (swamp.elements[e.id].className = "gg");
            });
          });
        },
        strikethrough(e, t) {
  ...
  * {
  ...
  .gg {
      background-color:rgb(155, 73, 255)
  }
  
  a {
      color:rgb(155, 73, 255);
      transition: color 250ms
  }
  
  a:hover {
      color: rgb(122, 52, 209)
  }`,
        ...
        run_code: {
          ...
          description:
            'Put your script here to run it while pretending to be the GoGuardian extension. You will be able to access most "chrome" scripts and have other privileges such as access to all websites. Note that your code is saved automatically. Developers: try interacting with the "swamp" object while running code on this page!',
          ...
          button_description:
            "Concerning the buttons above: Running on this page is pretty self explanatory. The script only takes effect when this page is open, but running as background lets the script run even with the tab closed. Basically, it means that the script is being run at the highest level of a Chrome extension, in the background, so it persists until Chrome is fully restarted (with chrome://restart for example).",
        },
        interesting_scripts: {
        ...
        ltbeef: {
          ...
          soft_disable_recommendation:
            "Disabling GoGuardian with this process will close the [swamp] launcher. As an alternative, use the soft-disable button earlier on the page, which has the same functionality while allowing for the launcher to be used.",
        },
        donations: {
          title: "Donations",
          description:
        ...
        confirm_reload:
          "It looks like you've got soft disable or tab-hiding active. Reloading the background scripts will also turn these off, so are you sure you want to continue?",
        confirm_remove_gg:
